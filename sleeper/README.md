...
type with unknown fields disallowed - a field Sleeper renames or drops fails the test instead
of silently zeroing out in prod. `TestAllFixturesCovered` also fails if a fixture has no case
exercising it, so a stale recording can't rot unnoticed.
...
- **Past season**: league `1257102049204514816` (2025, complete) - league settings, rosters,
  winners bracket, matchups for weeks 1 and 14, week-1 transactions, and its draft
...
- Players dump (`/v1/players/nfl`, ~14 MB live): trimmed to 50 entries - all 11 DEF (team
  defense) units on the current league's rosters, plus the first 39 rostered skill players by
  numeric player_id.
- Week-2 projections/stats: trimmed to the current league's 143 rostered player_ids plus the
  top 30 unrostered free agents by `pts_ppr` projection.
...
`cmd/qa-mock serve` (no `--record`) replays only what's already recorded and 404s on a miss,
so CI and `client_test.go` never touch the real API.
...
at `cmd/qa-mock` instead of the real API.
